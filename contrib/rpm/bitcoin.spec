...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 7832
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 7833
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 17832
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 17833
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 7832
	%{_sbindir}/semanage port -d -p tcp 7833
	%{_sbindir}/semanage port -d -p tcp 17832
	%{_sbindir}/semanage port -d -p tcp 17833
	for selinuxvariant in %{selinux_variants}; do
