MTTcoin Core version *0.15.1* is now available from:

  <https://download.MTTcoin.org/MTTcoin-0.15.1/>
...
A number of changes to the way MTTcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 MTcent per kB (0.00001 MTT). This is to prepare for dropping the minimum transaction fee to 0.1 MTcent per kB (0.0001 MTT) in the next release.
